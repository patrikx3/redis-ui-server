...
const disconnectRedisIo = (options) => {
    ...
    if (Array.isArray(socket.p3xrs.tunnels) && socket.p3xrs.tunnels.length > 0) {
        for (const server of socket.p3xrs.tunnels) {
            server.close()
        }
        socket.p3xrs.tunnels = []
    }
    if (socket.p3xrs.sshClient) {
        socket.p3xrs.sshClient.end()
        socket.p3xrs.sshClient = undefined
    }
}
...
const parseModuleList = (rawList) => {
    ...
    for (const entry of rawList) {
        ...
        for (let i = 0; i < entry.length; i += 2) {
            mod[entry[i]] = entry[i + 1]
        }
        if (mod.name) modules.push(mod)
    }
    return modules
}

const detectModules = async (redis) => {
    try {
        const rawList = await redis.call('MODULE', 'LIST')
        return parseModuleList(rawList)
    } catch (e) {
        // MODULE LIST not supported or disabled
        return []
    }
}
